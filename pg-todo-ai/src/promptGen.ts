...
export const generatePrompt = async (userQuery: string, requestTime: string) => {
  ...
  return generatePromptInternal(fnSchemas, sampleResponses, userQuery, requestTime);
...
const getRelatedSamplePromptResponses = async (query: string): Promise<SampleResponse[]> => {
  ...
  return await new Promise<SampleResponse[]>((resolve, reject) => {
    fs.createReadStream(filePath)
      ...
      .on('end', async () => {
        const embeddingsWithScore = await Promise.all(
        ...
        return resolve(
          result.map((r) => ({
            response: removeEscapedDoubleQuotesCsv(r.response),
            prompt: removeEscapedDoubleQuotesCsv(r.prompt),
          }))
        );
      });
  });
