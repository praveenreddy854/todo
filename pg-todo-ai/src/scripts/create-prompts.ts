...
export const createPrompts = () => {
  Object.entries(FnSchemas).forEach(([fnSchemaKey, fnSchemaVal]) => {
    ...
    const samples = fnSchemaVal.functions.map((fn) => '\n' + JSON.stringify(Samples[fn.functionName]));
    ...
    const rawInput: LlmRawInput = {
      ...
      functions: fnSchemaVal.functions.map((fn) => ({
        ...
        parameters: {
          ...
          required: fn.arguments.filter((arg) => arg.required).map((arg) => arg.name),
        },
      })),
    };
    ...
    // write to file
    fs.writeFileSync(`./dist/samples/prompts/${fileName}.json`, JSON.stringify(rawInput));
  });
};
