import { ContentFilterResult } from '@azure/openai';
import { ChatCompletionsWithPrompt } from './types/types';

export const getFilteredResponse = (response: ChatCompletionsWithPrompt): ChatCompletionsWithPrompt => {
...
const throwIfNotSafe = (key: string, contentFilterResult: ContentFilterResult) => {
  if (contentFilterResult.severity !== 'safe') {
    throw new Error(`It seems like the input has ${key} content. Please try again with a different input.`);
  }
};
